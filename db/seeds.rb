...
(0..5).each do |i|
  ...
  l = LabAssistant.create(first_name: la, last_name: 'Doe', email: la+'@test.com', password: 'password')
...
(0..6).each do |i|
  a = LabTime.create(open: Time.parse(times[i][0]), close: Time.parse(times[i][1]), day: days_of_week[days_of_week.keys[i]])
end
